...
def model(num_classes=20, input_size=(1024, 2048, 3)):
    ...
    conv1 = conv_layer(inputs, 32, 3)
    conv2 = conv_layer(conv1, 32, 3, separable=True)
    pool1 = MaxPooling2D(pool_size=(2, 2))(conv2)
    conv3 = conv_layer(pool1, 64, 3)
    conv4 = conv_layer(conv3, 64, 3, separable=True)
    pool2 = MaxPooling2D(pool_size=(2, 2))(conv4)
    conv5 = conv_layer(pool2, 128, 3)
    conv6 = conv_layer(conv5, 128, 3, separable=True)
    pool3 = MaxPooling2D(pool_size=(2, 2))(conv6)
    conv7 = conv_layer(pool3, 256, 3)
    conv8 = conv_layer(conv7, 256, 3, separable=True)
    drop1 = Dropout(0.5)(conv8)
    pool4 = MaxPooling2D(pool_size=(2, 2))(drop1)

    # The middle
    conv9 = conv_layer(pool4, 512, 3, separable=True)
    conv10 = conv_layer(conv9, 512, 3, separable=True)
    drop2 = Dropout(0.5)(conv10)

    # Decoding/Scaling Up
    up1 = conv_layer(UpSampling2D(size=(2, 2))(drop2), 256, 2, separable=True)
    merge1 = concatenate([drop1, up1], axis=3)
    conv11 = conv_layer(merge1, 256, 3, separable=True)
    conv12 = conv_layer(conv11, 256, 3, separable=True)

    up2 = conv_layer(UpSampling2D(size=(2, 2))(conv12), 128, 2, separable=True)
    ...
    up3 = conv_layer(UpSampling2D(size=(2, 2))(conv14), 64, 2, separable=True)
    merge3 = concatenate([conv4, up3], axis=3)
    conv15 = conv_layer(merge3, 64, 3, separable=True)
    conv16 = conv_layer(conv15, 64, 3, separable=True)

    up4 = conv_layer(UpSampling2D(size=(2, 2))(conv16), 32, 2, separable=True)
    merge4 = concatenate([conv2, up4], axis=3)
    conv17 = conv_layer(merge4, 32, 3, separable=True)
    conv18 = conv_layer(conv17, 32, 3, separable=True)
